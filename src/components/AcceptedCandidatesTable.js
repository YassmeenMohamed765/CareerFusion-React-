...
const AcceptedCandidatesTable = ({ postId }) => {
  ...
  const handleExportToExcel = async () => {
    try {
      const response = await axios.get(`http://localhost:5266/api/CVUpload/export-telephone-interview-passed/${postId}`, {
        responseType: 'blob',
      });
...
export default AcceptedCandidatesTable;
